...
TEST_CASE("Input angles wrap to PI", "[Angles.h]") {
  // test first quadrant
...
TEST_CASE("Input angles wrap to 2PI", "[Angles.h]") {
  // test first quadrant
  REQUIRE(beam::WrapToPi((0.25+0)*PI) == Approx(0.25*PI).epsilon(0.0001));
  REQUIRE(beam::WrapToPi((0.25+2)*PI) == Approx(0.25*PI).epsilon(0.0001));
  REQUIRE(beam::WrapToPi((0.25+4)*PI) == Approx(0.25*PI).epsilon(0.0001));
...
TEST_CASE("Test getting error between two angles", "[Angles.h]") {
...
TEST_CASE("Degrees to radians", "[Angles.h]") {
...
TEST_CASE("Radians to degrees", "[Angles.h]") {
