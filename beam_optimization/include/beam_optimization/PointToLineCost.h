...
struct CeresPointToLineCostFunction {
  ...
  bool operator()(const T* const T_REF_TGT, T* residuals) const {
     ...
     */
    T d1[3], d2[3], d12[3];
    d1[0] = P_REF[0] - static_cast<T>(P_REF1_[0]);
    d1[1] = P_REF[1] - static_cast<T>(P_REF1_[1]);
    d1[2] = P_REF[2] - static_cast<T>(P_REF1_[2]);

    d2[0] = P_REF[0] - static_cast<T>(P_REF2_[0]);
    d2[1] = P_REF[1] - static_cast<T>(P_REF2_[1]);
    d2[2] = P_REF[2] - static_cast<T>(P_REF2_[2]);
