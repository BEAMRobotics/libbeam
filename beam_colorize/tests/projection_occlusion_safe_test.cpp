...
TEST_CASE("column occlusion") {
  ...
  int count = 0;
  struct timespec t;
  for (int window_size = 5; window_size < 200; window_size += 5) {
    std::vector<int> strides{window_size * 0.25, window_size * 0.5,
                             window_size * 0.75};
    for (int stride : strides) {
      for (double depth_thres = 0.3; depth_thres < 0.6; depth_thres += 0.1) {
        count++;
        ...
        pcl::transformPointCloud(*map_colored_in_cam, *map_colored, T_MAP_CAM);
        *map_colored = beam::AddFrameToCloud(*map_colored, T_MAP_CAM);
        SaveMap(*map_colored, "test" + std::to_string(count) + ".pcd");
        int count_colored{0};
        ...
        double accuracy = static_cast<double>(count_colored) /
