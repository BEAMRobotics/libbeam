...
// world_frame_conversions header to produce ros tf::Transforms
...
class rosTfHelpersTest : public ::testing::Test {
 protected:
    double equatorial_radius = 6.3781370e6;
    double half_power = 1 / sqrt(2.0);

    void checkTfEnuEcefForLat0Lon0(const tf::Transform &tr) {
    ...
    void checkTfEcefEnuForLat0Lon0(const tf::Transform &tr) {
        EXPECT_NEAR(0, tr.getBasis().getRow(0).getX(), 1e-12);
    ...
    void checkTfEnuEcefForLat0Lon45(const tf::Transform &tr) {
        // X = [-0.707, 0, 0.707]^T
        // Y = [0.707, 0, 0.707]^T
        // Z = [0, 1, 0]^T
    ...
    void checkTfEcefEnuForLat0Lon45(const tf::Transform &tr) {
        // Invert above
        EXPECT_NEAR(-half_power, tr.getBasis().getRow(0).getX(), 1e-12);
        EXPECT_NEAR(0, tr.getBasis().getRow(0).getY(), 1e-12);
...
TEST_F(rosTfHelpersTest, Lat0Lon0Alt0Navsat) {
    sensor_msgs::NavSatFix datum_navsat;
    datum_navsat.latitude = 0;
    datum_navsat.longitude = 0;
    datum_navsat.altitude = 0;
    tf::Transform result_enu_ecef = makeTfENUFromECEF(datum_navsat);
    tf::Transform result_ecef_enu = makeTfECEFFromENU(datum_navsat);
...
TEST_F(rosTfHelpersTest, Lat0Lon0Alt0LLH) {
    double datum_llh[3] = {0, 0, 0};
    tf::Transform result_enu_ecef = makeTfENUFromECEF(datum_llh);
    tf::Transform result_ecef_enu = makeTfECEFFromENU(datum_llh);
...
TEST_F(rosTfHelpersTest, Lat0Lon0Alt0ECEF) {
    double datum_ecef[3] = {equatorial_radius, 0, 0};
    tf::Transform result_enu_ecef = makeTfENUFromECEF(datum_ecef, false);
    tf::Transform result_ecef_enu = makeTfECEFFromENU(datum_ecef, false);
...
TEST_F(rosTfHelpersTest, Lat0Lon45Alt0Navsat) {
    sensor_msgs::NavSatFix datum_navsat;
    datum_navsat.latitude = 0;
...
TEST_F(rosTfHelpersTest, Lat0Lon45Alt0LLH) {
...
TEST_F(rosTfHelpersTest, Lat0Lon45Alt0ECEF) {
    double datum_ecef[3] = {
