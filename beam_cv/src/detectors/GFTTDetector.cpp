...
GFTTDetector::GFTTDetector(int max_corners, double quality_level,
                           double min_distance, int block_size,
                           bool use_harris_detector, double k) {
  params_.max_corners = max_corners;
...
std::vector<cv::KeyPoint> GFTTDetector::DetectFeatures(const cv::Mat& image) {
  ...
  if (params_.max_corners != 0) {
    cv::KeyPointsFilter::retainBest(keypoints, params_.max_corners);
  }
  return keypoints;
}
