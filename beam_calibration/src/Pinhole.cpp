...
void Pinhole::AddRadDist(beam::VecX rad_coeffs){
  ...
  if(rad_coeffs_size < 3 || rad_coeffs_size > 6){
    LOG_ERROR("Invalid number of radial distortion coefficients. Input: %d, Min %d, Max: %d", rad_coeffs_size, 3, 6);
  } else {
...
beam::Vec2 Pinhole::ProjectPoint(beam::Vec4 X){
  ...
  if(is_full_ && homographic_form){
    ...
    LOG_ERROR("invalid entry, cannot project point: the point is not in homographic form, ");
  }
...
beam::Vec2 Pinhole::ApplyDistortedProjection(beam::Vec3 X){
  beam::Vec2 coords;
  beam::Vec3 x_proj;
  double x, y, xx, yy, r2, fx, fy, cx, cy, k1, k2, k3, k4=0, k5=0, k6=0, p1, p2;
  ...
  r2 = x*x+y*y;
  xx = x*((1+k1*r2+k2*r2*r2+k3*r2*r2*r2)/(1+k4*r2+k5*r2*r2+k6*r2*r2*r2))
       + 2*x*y + p2*(r2 + 2*x*x);
  yy = y*((1+k1*r2+k2*r2*r2+k3*r2*r2*r2)/(1+k4*r2+k5*r2*r2+k6*r2*r2*r2))
